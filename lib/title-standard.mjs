...
// ---------------------------------------------------------------------------
export const ALLOWED_TYPES = ["note", "task", "idea", "shopping", "reference", "person", "event"];
...
//
//   score >= 8  (and the call is confident)  -> archive + tag 'junk' + audit
//   score 5..7  (or 8+ but unsure)           -> KEEP + needs_review 'possible-junk'
//   score <  5                               -> keep, nothing said
//
// Nothing is ever deleted, at any score. Archiving is reversible from the audit
// entry, and an archived item still shows in the import deck.
// ---------------------------------------------------------------------------
export const JUNK_ARCHIVE_SCORE = 8; // the ruthlessness dial itself
export const JUNK_REVIEW_SCORE = 5; // below the archive bar but worth a glance

// A junk verdict at or above this confidence is acted on directly. Below it the
// call is treated as uncertain: keep + flag, never archive on a guess.
...
${JUNK_ARCHIVE_SCORE}+ means it gets archived. Score honestly; do not round up to tidy up.
...
- Always write a proper title, even at 10 — an archived item is still browsed.
- If you score ${JUNK_ARCHIVE_SCORE}+ but are not sure, keep "confidence" below
  ${JUNK_CONFIDENCE_BAR}: that routes it to the owner instead of archiving it.`;
...
export function titleQualityIssues(title, body = "") {
  ...
  if (nt.length >= 18 && nb.length >= nt.length && nb.startsWith(nt)) issues.push("raw-first-line");
...
/**
 * Combine the model's junk call with the structural one and turn it into the
 * single decision the pipelines act on.
 *
 *   verdict 'archive' -> status archived + tag 'junk' + audit 'junk_archived'
 *   verdict 'review'  -> item KEPT, needs_review = true, 'possible-junk'
 *   verdict 'keep'    -> nothing said
 *
 * Who decides:
 *   • a CERTAIN structural verdict (empty / test string) wins outright — a model
 *     claiming an empty note is valuable is simply wrong;
 *   • otherwise the MODEL decides, at its own confidence — it is the only thing
 *     that can tell a meaningless formula from the deal math;
 *   • with no model at all (sensitive captures never see one) the structural
 *     score stands but at zero confidence, so it can only ever reach 'review'.
 */
export function scoreJunk({ modelScore = null, modelConfidence = null, title = "", body = "" } = {}) {
  ...
  if (structural.certain) {
    ...
    confidence = 0; // no model, no certain structure -> never archive
  }

  let verdict;
  if (score >= JUNK_ARCHIVE_SCORE && confidence >= JUNK_CONFIDENCE_BAR) verdict = "archive";
  else if (score >= JUNK_REVIEW_SCORE) verdict = "review";
  else verdict = "keep";

  return { score, confidence, verdict, structuralReason: structural.reason };
}
...
function unreadableItem(originalText, reason) {
  ...
  return {
    title: usable ? derived : "Untitled capture",
    type: "note",
    body,
    tags: [],
    priority: "medium",
...
function collapsedSplit(items, originalText, confidence) {
  ...
  return {
    ...
    junk_score: junk.score,
    // Never archive something we already admit we misread.
    junk_verdict: junk.verdict === "archive" ? "review" : junk.verdict,
    junk_reason: junk.structuralReason,
  };
}
...
function sanitizeEnrichItem(it, fallbackText, fallbackConfidence) {
  ...
  // Junk pass. 'review' adds the flag but never overrides a more specific
  // reason already set above; 'archive' is acted on by lib/capture-core.ts.
  ...
  if (junk.verdict === "review" && !reviewReason) reviewReason = "possible-junk";
  ...
  return {
    ...
    confidence,
    // An archived-as-junk item is DECIDED, not pending: it must not also sit in
    // the review queue. Everything else keeps the normal review rules.
    needs_review: junk.verdict === "archive" ? false : belowBar || !!reviewReason,
    review_reason: junk.verdict === "archive" ? null : reviewReason,
    junk_score: junk.score,
    junk_verdict: junk.verdict,
