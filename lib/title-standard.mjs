...
- PARALLEL PARTS GET THE SAME TYPE. When the parts are the same kind of thing —
  three things to do, three people to contact — they are all tasks. A split that
  returns "task, memory, task" for three to-dos has mis-typed the middle one;
  the owner asked for one list, not a mixture. Past-tense phrasing in a dictated
  note ("called Jamie", "spoke to Nate") is usually a TRANSCRIPTION artefact of
  "call Jamie" — if the siblings are to-dos, this one is too.
