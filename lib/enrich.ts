...
export function buildEnrichSystemPrompt(todayISO: string, directive = ""): string {
  return [
    ...
    `     "due_date": "YYYY-MM-DD" if a deadline is implied, else null. Resolve`,
    `                 relative and FUZZY dates against today rather than giving up:`,
    `                   "end of August"       -> the last day of that August`,
    `                   "start of September"  -> the 1st`,
    `                   "mid-October"         -> the 15th`,
    `                   "next week"           -> 7 days from today`,
    `                   "Sunday" / "Friday"   -> the next such weekday`,
    `                 A named month already past this year means NEXT year. A vague`,
    `                 wish with no time in it ("some day", "eventually") is null —`,
    `                 the point is to catch real deadlines, not to invent them,`,
